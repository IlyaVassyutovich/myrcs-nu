...
# PROMPTS
$env.PROMPT_MULTILINE_INDICATOR = (^"C:/Program Files (x86)/oh-my-posh/bin/oh-my-posh.exe" print secondary $"--config=($env.POSH_THEME)" --shell=nu $"--shell-version=($env.POSH_SHELL_VERSION)")
...
if "false" == "true" {
    $env.TRANSIENT_PROMPT_COMMAND = { ||
        ^"C:/Program Files (x86)/oh-my-posh/bin/oh-my-posh.exe" print transient $"--config=($env.POSH_THEME)" --shell=nu $"--shell-version=($env.POSH_SHELL_VERSION)" $"--execution-time=(posh_cmd_duration)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=(posh_width)"
    }
}
